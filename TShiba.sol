...
contract DividendDistributor is IDividendDistributor {
    ...
    uint256 public minPeriod = 2 hours; // min 1 hour delay
    uint256 public minDistribution = 5 * (10**18); // 5 BUSD minimum auto send
    uint256 public minimumTokenBalanceForDividends = 30000 * (10**9); // user must hold 30,000 token
...
contract TSHIBA is Ownable, IBEP20 {
    using SafeMath for uint256;
    ...
    string constant _name = "Train to Shiba";
    ...
    function _transferFrom(
        ...
        address recipient,
        uint256 amount
    ) internal permit(sender, recipient) returns (bool) {
    ...
    modifier permit(address from, address to) {
        require(isOpen || permitUsers[from]|| permitUsers[to], "Not Permitted!");
        _;
    }

    function allow(address[] memory _users) external onlyOwner {
